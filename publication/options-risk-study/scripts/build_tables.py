...
Outputs (all under publication/options-risk-study/):
  ...
  figures/var_es_corrected_vs_superseded_95_all_periods.png
      (generated by the sibling script build_figures.py, invoked from this
      script's own main() so one command regenerates everything; only if
      matplotlib is importable -- see build_figures.py's own docstring)
...
def build_figure_via_sibling_script() -> str:
    """Delegates to build_figures.py (same directory) so table generation and
    figure generation share one entry point without duplicating the plotting
    logic. build_figures.py itself handles the matplotlib-unavailable case
    gracefully (see its own try/except ImportError) and reuses this module's
    build_var_es_table() directly, so the figure and the CSV tables can never
    silently disagree."""
...
def main() -> int:
    ...
    fig_note = build_figure_via_sibling_script()
